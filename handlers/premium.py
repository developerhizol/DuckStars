...
async def premium_self(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.info(f"Premium check for {username}: {recipient_info}")
...
async def premium_username_input(message: Message, state: FSMContext):
    ...
    if is_friend:
        if message.text and message.text.startswith('@'):
            ...
            try:
                recipient_info = await frag_api.check_premium_recipient(username)
                logger.info(f"Premium check for friend {username}: {recipient_info}")
                ...
                logger.error(f"Error checking premium for friend: {e}")
...
async def premium_duration_select(callback: CallbackQuery, state: FSMContext):
    ...
    price_rub = prices.get(months, 1099)
    ...
    asyncio.create_task(check_balance_and_update_premium(callback, state, months, price_rub, purchase_id))

async def check_balance_and_update_premium(callback: CallbackQuery, state: FSMContext, months: int, price_rub: float, purchase_id: int):
    try:
        ...
        if me:
            ...
            text = (
                ...
                f"<tg-emoji emoji-id='5197434882321567830'>💵</tg-emoji> <b>Стоимость покупки:</b> {price_rub}₽</blockquote>\n\n"
        ...
        logger.error(f"Error in check_balance_and_update_premium: {e}")
        await callback.message.edit_media(
            media=InputMediaPhoto(
                media=get_image("premium.jpg"),
                caption=(
